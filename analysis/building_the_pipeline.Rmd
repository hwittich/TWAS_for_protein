...
sig_hits<-mutate(sig_hits,cis_trans = ifelse((new_gene_start-500)<=new_protein_start & new_protein_start<=(new_gene_start+500),"cis-acting","trans-acting"))
```
...
```{r}
library(ggplot2)
ggplot(sig_hits,aes(x=new_gene_start,y=new_protein_start,size=pvalue,color=cis_trans)) + 
  geom_point(shape=21) + 
