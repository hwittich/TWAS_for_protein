...
for filename in os.listdir(models):
    if filename[-3:] == ".db":
        ...
        ##Perform predict.py
        os.system("python3 code/transcript_prediction.py --dose "+dosages+" --exp "+models+filename+" --samples "+samples+" --out "+outfile+tissue+"/"+tissue+"_predict")
        ...
        for SL_ID in SL_IDs: #Loop through every protein
            association_command = subprocess.Popen(["python3","code/protein_association.py","--pred",outfile+tissue+"/"+tissue+"_predict.txt","--prot",SL_ID+".txt","--tissue",tissue,"--out",outfile])
